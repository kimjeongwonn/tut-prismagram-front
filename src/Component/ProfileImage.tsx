import React from 'react';
import styled from 'styled-components';

const Image = styled.div<{ img: string | null; size: number }>`
  width: ${({ size }) => `${size}px`};
  height: ${({ size }) => `${size}px`};
  clip-path: circle(${({ size }) => `${size}px`} at center);
  background-image: url(${({ img }) =>
    img ||
    'data:image/svg+xml;base64,'});
  background-size: contain;
  margin: 0 5px;
`;

export default ({ img, size = 32 }: { img: string | null; size: number }) => {
  return <Image img={img} size={size} />;
};
